...
| 2.AG | **TIMG (Timer Group) peripheral** at `0x500BC000`. 54-bit counter @ 1 MHz tick anchored to QEMU_CLOCK_REALTIME. Alarm comparator + 50ms QEMUTimer + JSON `event:"timg"`. Self-test pre-program produces 1-Hz autoreload events out-of-the-box. Foundation for Arduino `hw_timer_t` / `attachInterrupt`. | ✅ done | (this commit) | [phase_2ag_timg.md](phase_2ag_timg.md) |
| **next** | TBD: 2.AH (TIMG → CPU IRQ wiring), I2C master, SPI master, real PWM waveform on GPIO, LEVEL_HIGH/LOW GPIO IRQ types, or real FreeRTOS port (Phase 2.V deferred) | ⏭️ choice | — | — |
