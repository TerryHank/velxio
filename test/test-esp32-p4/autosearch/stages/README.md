...
| 2.T-fix.next | **Phase 2.M typo fix**: `0xFA4FC06F` → `0xFA5FC06F` (1-bit `imm[11]` error in JAL encoding). Was jumping to `0x3FFFF820` instead of intended `0x40000020 (setup())`. Now bypass-dropped flow reaches setup(), panic handler completes full output incl. Reboot sequence. | ✅ done | (this commit) | [phase_2t_fix_next_typo.md](phase_2t_fix_next_typo.md) |
| **2.T-fix.next.next** | **Identify NULL+12 dereference at 0x4000079A** in setup() — likely `partition->flash_chip->chip_drv` deref. Provide fake `esp_flash_t`. | ⏭️ **next** | — | TBD |
| 2.U | Drop Phase 2.M-2.O bypass patches, run natural Arduino flow (depends on 2.T-fix.next.next) | ⏳ planned | — | TBD |
