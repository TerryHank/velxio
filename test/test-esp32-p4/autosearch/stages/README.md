...
| 2.AY | **🎉 RMT (Remote Control) — WS2812 NeoPixel** at `0x500A4000`. First brand-new peripheral since the WDT chain. Decodes RMT item streams (32-bit `{lvl1,dur1,lvl0,dur0}` pulses) as WS2812B bits via `dur0 > dur1` heuristic — timing-invariant, no clock-rate plumbing needed. 24 bits packed in GRB order per pixel, JSON event `{"event":"rmt","ch":N,"pixel":P,"r":R,"g":G,"b":B}` per pixel. Self-test at boot fires 3 pixels (pure red/green/blue) validating the full path. Foundation for Adafruit_NeoPixel / FastLED Arduino demos. **17 JSON event types now**. | ✅ done | (this commit) | [phase_2ay_rmt.md](phase_2ay_rmt.md) |
| **next** | TBD: 2.AY.arduino-real (validate real NeoPixel sketch), TWAI (CAN), WDT actual reset, multi-UART, real PWM, FreeRTOS (deferred) | ⏭️ choice | — | — |
