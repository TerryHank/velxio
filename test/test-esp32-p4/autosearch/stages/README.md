...
| 2.AL | **Multi-source ISR (TIMG + GPIO)** — single ISR dispatches on `mcause` to handle both TIMG (cause 19, toggles pin 8) and GPIO (cause 18, W1TC INT_STATUS + toggles pin 9). 21-instruction ISR with branch-chain dispatch. Real-silicon shared-trap-vector pattern. | ✅ done | (this commit) | [phase_2al_multi_source_isr.md](phase_2al_multi_source_isr.md) |
| **next** | TBD: TIMG1 + WDT, I2C master (BMP280 demos), SPI master, real PWM waveform on GPIO via LEDC, UART RX path, or real FreeRTOS port (Phase 2.V deferred) | ⏭️ choice | — | — |
