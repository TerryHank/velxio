...
| 2.BC | **Synthetic TWAI RX responder — bidirectional CAN bus**. Mirrors BMP280 I2C / ILI9341 SPI responder pattern: pre-loads canned frame (ID=0x456, DLC=3, data=[0xCA,0xFE,0x42]) into RX buffer, sets STATUS.RX_BUFFER bit; CMD.RELEASE_RX writes consume frame + emit `{"event":"twai_rx",...}` JSON event AND auto-load next rotating-ID frame so successive polls see fresh data. Boot self-test exercises both TX (Phase 2.BA) and RX (new) — round-trip ID encode/decode verified for both directions. Closes the CAN loop: Arduino `twai_receive()` sketches now work. **19 JSON event types**. | ✅ done | (this commit) | [phase_2bc_twai_rx.md](phase_2bc_twai_rx.md) |
| **next** | TBD: TWAI IRQ wiring, TWAI1/2 instantiation, WDT actual reset, real PWM, FreeRTOS (deferred) | ⏭️ choice | — | — |
