...
| 2.X.input | **Reverse channel**: `VELXIO_GPIO_INPUT=/path/to/fifo`. Frontend writes `{"pin":N,"level":M}` lines; emulator parses + forwards to `external_input` pads. Verified: 4 frontend-injected events appear in output log with 500 ms wall-clock spacing. **Bidirectional emulator ↔ frontend channel complete**. | ✅ done | (this commit) | [phase_2x_input_reverse_channel.md](phase_2x_input_reverse_channel.md) |
| **2.Y** | **Real SYSTIMER-based delay** (replace busy-wait with virtual-time reads — deterministic timing) | ⏭️ **next** | — | TBD |
